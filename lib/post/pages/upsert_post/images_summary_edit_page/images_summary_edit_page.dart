...
class ImagesSummaryEditPage extends ConsumerStatefulWidget {
  ...
  ImagesSummaryEditPage({Key? key, required this.showCameraOptionsOnEnter}) : super(key: key);
...
class _ImagesSummaryEditPageState extends ConsumerState<ImagesSummaryEditPage> {
                ...
                children: [
                  AspectRatio(
                    ...
                    child: PageView(
                        ...
                        children: store.displayImagePaths.map((image) => PostImage(imageUrl: image)).toList()),
                  ),
                  const SizedBox(
                    height: 8,
                  ),
  ...
  Widget buildPhotoListItem({required int index}) {
    ...
    return Stack(
        children: [
          GestureDetector(
            onTap: () => imagePageController
                .animateToPage(index, duration: const Duration(milliseconds: 500), curve: Curves.ease),
